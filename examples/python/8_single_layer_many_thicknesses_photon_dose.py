...
mc_results = pkc.compute_buildup(
    geometries=mc_geometries, source=source, quantities=["dose-AP"],
    particles_per_batch=10_000, max_batches=50, trigger_rel_err=0.05,
    cross_sections="~/nuclear_data",
)
...
ax.errorbar(mc_thicknesses, mc_doses, yerr=mc_errs, fmt="ko", markersize=7,
            capsize=4, zorder=5, label="Monte Carlo simulation")
...
output_path = os.path.join(os.path.dirname(__file__), "8_single_layer_many_thicknesses_photon_dose.png")
