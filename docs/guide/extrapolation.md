...
for t in all_thicknesses:
    layers = [pkc.Layer(thickness=t, material=concrete)]
    bi = table.interpolate(thickness=t)

    result = pkc.calculate_dose(SOURCE, layers, source, "AP", buildup=bi)
...
result = pkc.calculate_dose(
    1e12,
    [pkc.Layer(thickness=50, material=concrete)],
    source, "AP",
    buildup=bi,
)
```
...
result = pkc.calculate_dose(
    ...
    buildup=pkc.BuildupModel.constant(my_B),
)
```
...
for t in all_thicknesses:
    layers = [pkc.Layer(thickness=t, material=concrete)]
    bi = table.interpolate(thickness=t)
    pk = pkc.calculate_dose(SOURCE, layers, source, "AP")
